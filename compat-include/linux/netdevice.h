...
/* Before the per-netdevice lock was introduced, RTNL serialized device
 * registration state changes.
 */
#define netdev_lock(dev) rtnl_lock()
#define netdev_unlock(dev) rtnl_unlock()

#endif /* LINUX_VERSION_CODE < KERNEL_VERSION(6, 14, 0) && RHEL_RELEASE_CODE == 0 */
...
#define dev_dstats_rx_dropped(dev) dev_core_stats_rx_dropped_inc(dev)
	...
	OVPN_SLE_VERSION_BEFORE(15, 5, 0) && \
	RHEL_RELEASE_CODE < RHEL_RELEASE_VERSION(8, 10)
...
#endif /* LINUX_VERSION_CODE < KERNEL_VERSION(5, 19, 0) && OVPN_SLE_VERSION_BEFORE(15, 5, 0) && RHEL_RELEASE_CODE < RHEL_RELEASE_VERSION(8, 10) */
